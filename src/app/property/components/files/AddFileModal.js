...
import PersonList from '../create/PersonList';
import { FileTypes, FileCategories } from '../../services';
...
//'/webviewer/lib' 07/07/20 - will check on the public folder access //post scan - components review 0818//link generator for non app users
...
const addFileWasabi = (file) => {
 ...
 // console.log('addFileWasabi params',params) -- //link generator for non app users
  ...
  s3.upload(params, function(err, data) 
  {
    ...
    //let { id, username } = $store.getState().User.user.user
      ...
      const bsonId  = new ObjectId(); //assetId for PDFtron fixes
...
const AddFileModal = props => {  
  ...
  const [state, setState] = useState({ fileName: 'New File',bucketname:props.bucketname });
  //const [bucketname, SetBucketname] = useState({...state,bucketname:props.bucketname});
  fs.useState(state, setState);  
 // console.log('AddFileModal props bucketname',state.bucketname)
