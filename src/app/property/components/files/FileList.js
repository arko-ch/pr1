...
import PdfViewerAnnot from './PdfViewerAnnot';

const $files = crud('files');

//const $files = crud('files');//rawi0713 //post scan - components review 0818
...
export default class FileList extends Component {
  ...
  componentDidMount() {
    //const currentUserName = window.sessionStorage.getItem('experiment');
    //const currentUserName = JSON.parse(sessionStorage.getItem('settlement-app-client')).user.data.user.name //window.sessionStorage.getItem('settlement-app-client')
    //JSON.parse(sessionStorage.getItem('settlement-app-client')).user.data.user.profilepic)
    //console.log('currentUserName',currentUserName)
    //console.log('this.props.files',this.props.files[0].assetId)
    //this.setState({attachmentId:this.props.files[0].assetId
    //console.log('this.props.id._id',this.props.id._id)
  }
  /* onDocumentLoadSuccess = ({ numPages }) => {
    this.setState({ numPages });
  }
 */
  //post scan - components review 0818
  remove(idx) {
  ...
  getFile(b, f, t) {
    ...
    let fName = pfname, //this.state.fileName,
      fType = pftype; //this.state.fileType; //post scan - components review 0818
  ...
  render() {
    ...
    const addFile = (file, text) => {
      ...
      let wasabi_left_extn = this.props.files[0].key.slice(
        ((this.props.files[0].key.lastIndexOf('.') - 1) >>> 0) + 2
      );
        ...
        if (err) {
          ...
          const requestOptions = {
            ...
            body: JSON.stringify({
              ...
              right: {
                ...
                file_type: data.Key.slice(
                  ((data.Key.lastIndexOf('.') - 1) >>> 0) + 2
                )
              }
            })
          };
          ...
          fetch('https://api.draftable.com/v1/comparisons', requestOptions)
            ...
            .then(data => {
              ...
              Swal.fire({
                title:
                  '<strong>Document <u>Successfully processed</u></strong>',
                icon: 'info',
                html: `<a target='_blank' href="//api.draftable.com/v1/comparisons/viewer/hNkPix-test/${viewid}">View</a>`,
                showCloseButton: true,
                // showCancelButton: true,
                focusConfirm: false,
                confirmButtonText: '<i class="fa fa-thumbs-up"></i> Exit'
              });
            });
        }
      });
    ...
    let bucketname;
    let currentUserNameF = JSON.parse(
      sessionStorage.getItem('settlement-app-client')
    ).user.data.provider.claims.profile.firstName; //user.name
    let currentUserNameL = JSON.parse(
      sessionStorage.getItem('settlement-app-client')
    ).user.data.provider.claims.profile.lastName;
    let currentUserName = currentUserNameF + ' ' + currentUserNameL;

    let files = this.state.files.map((f, idx) => {
      //      console.log('files schema and this. props',f,this.props);
      let fhref = 'https://docs.google.com/viewerng/viewer?url=' + f.wasabi;
      //let fviewer=f.wasabi
      ...
      return (
        <li key={`file-${idx}`}>
          ...
          {/*  <button className="btn btn-link"  
            onClick={() => { // this.view(idx); //this.getFile(f.bucket,f.key,f.filetype)}} 
          >
            view
          </button> */}
          {/* `https://${bucket}.s3.wasabisys.com/${fn}`  https://docs.google.com/viewerng http://docs.google.com/gview */}
          ...
          <div>
            <PdfViewerAnnot
              docId={f.assetId}
              wasabi={f.wasabi}
              annotationUser={currentUserName}
            />
            {/*  {
          ...
          {draftableIdentifier === 'undefined' ? null : (
            <span style={{ color: '#1e8bff' }}>
              <a target="_blank" href={draftableIdentifier}>
                {' '}
                View Comparison
              </a>
            </span>
...
//for simplify tracker -> tbd 10/19
