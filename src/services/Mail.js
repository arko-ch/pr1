...
import $root from '../app/services/events.js';
import $http from '../app/services/http.js';
...
class Mail {
  ...
  async getAttachment(msgId, attachId) {
    return await $http.get( entryPoint + '/messages/' + msgId + '/attachments/' + attachId);
  }
  
  async getInLineAttachmentsa(msgId,attachId) {
    console.log('mail services=> getInLineAttachmentsa' + 'msgid->>'+ msgId +'--attachedid-->'+attachId)
    //console.log('ito yung ENTRY POINT',JSON.stringify($http.get(entryPoint + '/messages/' + msgId + '/attachments/?$expand=microsoft.graph.itemattachment/item')))
    return await $http.get(
      entryPoint +
        '/messages/' +
        msgId +
        '/attachments/' + attachId //?$expand=microsoft.graph.itemattachment/item'
    );
  }
  ...
  console.log('writeintcoms... msgid->>'+ message.id + '====writeintcoms====>' + message.comsInternal);
     return await $http({
  ...
  async getBubbles(msgId) {
    return await $http.get(
      entryPoint + '/messages/' + msgId + '/extensions/' + 'Com.Settlementapp99.Bubble'
    );
  }
  
  async getAvatars(msgId) {
    return await $http.get(
      entryPoint + '/messages/' + msgId + '/extensions/' + 'Com.Settlementapp99.Viewers'
    );
  }
  ...
  async kuninInlineatta(msgId,attachId) {
    console.log('mail services=> kuninmethod' + 'msgid->>'+ msgId +'--attachedid-->'+attachId) //'ito yung ENTRY POINT',JSON.stringify($http.get(entryPoint + '/messages/' + msgId + '/attachments/?$expand=microsoft.graph.itemattachment/item')))
    return await $http.get(entryPoint + '/messages/' +  msgId +  '/attachments' + attachId +'/?$expand=microsoft.graph.itemattachment/item');
  }
}
